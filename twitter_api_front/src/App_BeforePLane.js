...
class App extends Component {
  ...
  state = {
    // array with the keys: source = source, title, description, id;
    // 
    tweets:[],
    blockedUser: "",
    tweetsLoaded:false,
...
componentDidMount() {
  ...
  blockUser = (userName, index) => {
   ...
   // blockedUser.id= id;
    // let activeCart = [...thisCart, myCart]
     ...
     {this.state.tweetsLoaded?
          ...
          {tweets.map((item,i) => {
               ...
               <blockquote  className="twitter-tweet" ref={this.tweet}>
                  {item.userName}   <p></p>
                 <p>{item.text}</p>
                 <button className={item.blocked?blkdUserButton:unblkdUserButton} name="flag" id="flag"  onClick={() => this.blockUser(item.userName, i)}> {item.blocked?blkdUserButtonText:unblkdUserButtonText} </button>
              </blockquote>
